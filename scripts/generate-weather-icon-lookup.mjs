...
const manifest = readFileSync(join(ROOT, 'arduino', 'assets', 'manifest.txt'), 'utf8')
  .split('\n')
...
const cpp = `#include "asset_icons.h"
...
writeFileSync(join(ROOT, 'arduino', 'asset_icons.cpp'), cpp);
console.log(`Wrote arduino/asset_icons.cpp (${icons.length} weather icons)`);
